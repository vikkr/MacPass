...
+ (NSArray *)recursivelyFindEntriesInGroups:(NSArray *)groups forURL:(NSString *)url {
  ...
  for (KPKGroup *group in groups) {
    ...
    [entries addObjectsFromArray:[MPDocumentQueryService recursivelyFindEntriesInGroups:group.groups forURL:url]];
    
    /* check each entry in the group */
    for (KPKEntry *entry in group.entries) {
      if (url == nil || [entry.title rangeOfString:url].location != NSNotFound || [entry.url rangeOfString:url].location != NSNotFound) {
        [entries addObject:[KPHResponseEntry entryWithUrl:entry.url name:entry.title login:entry.username password:entry.password uuid:[entry.uuid UUIDString] stringFields:nil]];
      }
    }
  }
...
- (NSArray *)server:(KPHServer *)server entriesForURL:(NSString *)url {
  ...
  return [MPDocumentQueryService recursivelyFindEntriesInGroups:self.queryDocument.root.groups forURL:url];
}
...
- (NSArray *)allEntriesForServer:(KPHServer *)server {
  ...
  return [MPDocumentQueryService recursivelyFindEntriesInGroups:self.queryDocument.root.groups forURL:nil];
}
